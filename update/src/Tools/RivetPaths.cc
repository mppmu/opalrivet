...
namespace Rivet {
  ...
  void setAnalysisRefPaths(const vector<string>& paths) {
    const string pathstr = pathjoin(paths);
    setenv("RIVET_REF_PATH", pathstr.c_str(), 1);
  }

  void setAnalysisPlotPaths(const vector<string>& paths) {
    const string pathstr = pathjoin(paths);
    setenv("RIVET_PLOT_PATH", pathstr.c_str(), 1);
  }

  void setAnalysisInfoPaths(const vector<string>& paths) {
    const string pathstr = pathjoin(paths);
    setenv("RIVET_INFO_PATH", pathstr.c_str(), 1);
  }
  ...
  void addAnalysisPlotPath(const string& extrapath) {
    vector<string> paths = getAnalysisPlotPaths();
    paths.push_back(extrapath);
    setAnalysisPlotPaths(paths);
  }


  void addAnalysisInfoPath(const string& extrapath) {
    vector<string> paths = getAnalysisInfoPaths();
    paths.push_back(extrapath);
    setAnalysisInfoPaths(paths);
  }


  void addAnalysisRefPath(const string& extrapath) {
    vector<string> paths = getAnalysisRefPaths();
    paths.push_back(extrapath);
    setAnalysisRefPaths(paths);
  }
