...
public:
  ...
  void init()
    ...
    { H_it->second->Sumw2(); H_it->second->Scale(1.0/fTotalWeight);  }
    ...
    for (std::map<std::string,TAdvancedGraph*>::iterator G_it=fGMap.begin(); G_it!=fGMap.end(); ++G_it) //if (G_it->first.find("JETR")!=std::string::npos) 
    {
