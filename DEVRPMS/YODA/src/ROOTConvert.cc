...
Histo1D* TH1toHisto1D(const TH1* th1) {
   std::vector<HistoBin1D> bins;
    ...
    for (int i = 0; i <= th1->GetNbinsX()+1; ++i) {
      Dbn1D dbn(static_cast<unsigned long>(th1->GetBinContent(i)), th1->GetBinContent(i), sumw2s->GetAt(i), 0, 0);
    ...
    Histo1D* rtn =new Histo1D(bins, dbn_tot, dbn_uflow, dbn_oflow, th1->GetName(), th1->GetTitle());
...
Scatter2D* TH1toScatter2D(const TH1* th1) {
   ...
   TH1D* Histi1DtoTH1D(const Histo1D* h) {
...
//FIXME      rtn->addPoint(x, p->GetBinContent(i)/width, exminus, explus,  p->GetBinErrorLow(i)/width, p->GetBinErrorUp(i)/width);
    ...
    rtn->addAnnotation("XLabel", p->GetXaxis()->GetTitle());
    rtn->addAnnotation("YLabel", p->GetYaxis()->GetTitle());
    return rtn;
...
TGraphAsymmErrors* Scatter2DtoTGraph(const Scatter2D* s) {
