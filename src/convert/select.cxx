...
for (std::map<std::string,TH1D*>::iterator H_it=fHMap.begin(); H_it!=fHMap.end(); ++H_it)
{
	...
	{ 
		...
		replace_all(name,"h986","H_corrected_data_"+std::string(argv[2])+"_207GeV_");
		replace_all(name,"h186","H_corrected_data_"+std::string(argv[2])+"_94GeV_");
...
fFile->cd();
    ...
    for (std::map<std::string,TH1D*>::iterator H_it=fHMap.begin(); H_it!=fHMap.end(); ++H_it) 
    if (std::string(H_it->second->GetName()).find("H_corrected_data_")!=std::string::npos) 
