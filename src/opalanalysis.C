    ...
    for (std::vector<std::string>::iterator it=S.begin(); it!=S.end(); it++)
        {
            ...
            if (!R) {printf("No such sample %s\n",it->c_str());  exit(1);}
                ...
                {                    
                    if (proc.find(R->fProcesses)==proc.end())
                    proc.insert(std::pair<std::string,std::pair<double,int> >(R->fProcesses,std::pair<double,int>(0.0,0)));
                    else { proc[R->fProcesses].first+=R->fLuminocity; proc[R->fProcesses].second+=R->fEvents; }  ///Sum the luminocity and number of events for all processes.                  
...
Bool_t opalanalysis::Notify()
{
    ...
    for (std::vector<std::string>::iterator it=S.begin(); it!=S.end(); it++)
     ...
     TSampleInfo* R=(TSampleInfo*)theDB->Get(it->c_str());
            ...
            if ((fSampleInfo->fEnergyString==R->fEnergyString)&&( std::find(R->fFiles.begin(), R->fFiles.end(), std::string(currentfile.Data()))!=R->fFiles.end() ))  
            {
